import { getEvergreenFallbackArticles, sanitizeArticleRecommendations } from "@/lib/articles";
...
export async function GET(request: Request) {
  try {
    ...
    if (!supabase) {
      return jsonSuccess(sanitizeArticleRecommendations(getEvergreenFallbackArticles()).slice(0, 3), 200);
    }
    ...
    return jsonSuccess(sanitizeArticleRecommendations(articles).slice(0, 3), 200);
  } catch (error) {
